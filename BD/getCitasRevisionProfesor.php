...
$query = "select '' as nombre, fecha, hora_inicio, hora_fin from citasrevision where idevaluacion = $idEval
except
SELECT '' as nombre,cr.fecha,cr.hora_inicio,cr.hora_fin 
FROM citasrevision_estudiantes AS cre 
INNER JOIN personas p ON cre.cedula=p.cedula
INNER JOIN citasrevision AS cr ON cr.idcita=cre.idcita 
WHERE cr.idEvaluacion=$idEval
union
SELECT p.nombre||' '||p.apellido1||' '||p.apellido2 as nombre,cr.fecha,cr.hora_inicio,cr.hora_fin 
FROM citasrevision_estudiantes AS cre 
INNER JOIN personas p ON cre.cedula=p.cedula
INNER JOIN citasrevision AS cr ON cr.idcita=cre.idcita 
WHERE cr.idEvaluacion=$idEval";
